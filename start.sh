...
if [ -n "$DATABASE_URL" ]; then
    ...
    if [ -z "$TABLE_EXISTS" ]; then
        ...
        psql "$DATABASE_URL" -f data/postgres/init/01_northwind_demo.sql || echo "Warning: database seed failed, continuing anyway"
        echo "Database seeded."
    fi
else
    echo "Warning: DATABASE_URL not set, skipping database seed"
fi
